...
public class DateUtil {
    ...
    /**
     * Parse RSS date format to Date object. Example of RSS date: Sat, 23 Sep
     * 2006 22:25:11 +0000
     */
    public static Date parseDate(String dateString) {
        ...
        try {
            ...
            if (columnCount == 5) {
                ...
                timeIndex = 5;
                gmtIndex = 6;
            } else if (columnCount == 6 && values[0].endsWith(",")) { //search.json
                //Mon, 09 Jul 2012 09:00:35 +0000
                dayOfMonthIndex = 1;
                monthIndex = 2;
                yearIndex = 3;
                timeIndex = 4;
    ...
    public static String strftime(String format, Date date) {
        Calendar cal = Calendar.getInstance();
        cal.setTimeZone(TimeZone.getDefault());
        if (date != null) {
            cal.setTime(date);
        }
        format = StringUtil.replace(format, "%H", "" + cal.get(Calendar.HOUR_OF_DAY));
        format = StringUtil.replace(format, "%M", "" + cal.get(Calendar.MINUTE));
        return format;
    }
}
